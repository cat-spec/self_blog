<template>
 ...
 <div v-if="isAiThinking" class="message-item other">
      🤖
      <div class="typing-indicator">思考中
        <span></span>
        <span></span>
        <span></span>
      </div>
...
const sendessage = async () => {
    ...
    console.log(res)
    message.value = ''
    AImessage.value.push(res.data.message)
...
.typing-indicator {
  ...
  background-color: #f0f2f5;  /* 与 AI 气泡背景一致 */
  border-radius: 18px;
  padding: 10px 16px;
  gap: 6px;
}
