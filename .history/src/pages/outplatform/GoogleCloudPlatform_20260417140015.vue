...
// 页面加载完成后初始化地图
onMounted(() => {
  const userkey="IXkFtHDFk0dZ8GXqYWXfMr3kyV8Tltz5"
  const userKey = 'YOUR_AK'; // 替换为您的浏览器端AK
    const script = document.createElement('script');
    script.src = `https://api.map.baidu.com/api?v=3.0&ak=${userKey}`; // 使用WebGL版本
...
function initMap() {
      ...
      if (window.BMapGL) {
        ...
        map.centerAndZoom(new BMapGL.Point(116.404, 39.915), 11); // 设置中心点和缩放级别
      }
}
