<template>
  <div>
     <div class="graph-card">
        ...
        <div id="mastery-graph" class="chart-container"></div>
...
const skillsData = {
    ...
    function renderMasteryGraph() {
        const container = document.getElementById('mastery-graph');
        if (!container) return;
    ...
    window.addEventListener('load', () => {
        renderMasteryGraph();
        console.log('✅ 熟练度关系网已加载 | 节点颜色: 橙(高) 蓝(中) 灰(低)');
    });

    // 可选：暴露更新数据的方法，方便控制台动态修改
    window.updateSkillsGraph = function(newNodes, newLinks) {
        if (newNodes && Array.isArray(newNodes)) {
            skillsData.nodes = newNodes;
        }
        if (newLinks && Array.isArray(newLinks)) {
            skillsData.links = newLinks;
        }
        renderMasteryGraph();
        console.log('🔄 技术栈图谱已更新');
    };
