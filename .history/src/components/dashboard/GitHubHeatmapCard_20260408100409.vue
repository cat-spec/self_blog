  ...
  <GlassCard>
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <div class="mt-2 text-xs opacity-70">支持年份切换（当前为示例数据，可对接 API 替换生成逻辑）</div>
      </div>
      ...
      <div class="w-[180px] shrink-0">
        ...
        <div class="mt-5">
          ...
          <div class="flex items-center gap-2 text-xs opacity-80">
            <span class="inline-flex w-3 h-3 rounded-[2px]" style="background:rgba(170, 59, 255, 0.3); opacity:0.55" />
            <span>低</span>
            <span class="inline-flex w-3 h-3 rounded-[2px]" style="background:rgba(170, 59, 255, 0.6); opacity:0.7" />
            <span>中</span>
            <span class="inline-flex w-3 h-3 rounded-[2px]" style="background:var(--accent); opacity:0.95" />
            <span>高</span>
          </div>
        </div>
      </div>
...
function levelToColor(level) {
  ...
  if (level === 1) return { backgroundColor: 'rgba(170, 59, 255, 0.3)', opacity: 0.55 }
  if (level === 2) return { backgroundColor: 'rgba(170, 59, 255, 0.6)', opacity: 0.7 }
  return { backgroundColor: 'var(--accent)', opacity: 0.95 }
}
